...
namespace TSPL_Driver_Printer\src;
...
abstract class TSPLImage
{
     ...
     * @return GdTSPLImage|ImagickTSPLImage|NativeTSPLImage
     * @throws Exception Where no suitable library could be found for the type of file being loaded.
     */
    public static function load(
        ...
        foreach ($preferred as $implementation) {
            if ($implementation === 'imagick') {
                ...
                return new ImagickTSPLImage($filename, $allowOptimisations);
            } elseif ($implementation === 'gd') {
                if (!self::isGdLoaded()) {
                    // Skip option if GD not loaded
                    continue;
                }
                return new GdTSPLImage($filename, $allowOptimisations);
                ...
                return new NativeTSPLImage($filename, $allowOptimisations);
            } else {
                // Something else on the 'preferred' list.
                throw new InvalidArgumentException("'$implementation' is not a known TSPLImage implementation");
            }
        }
        throw new InvalidArgumentException("No suitable TSPLImage implementation found for '$filename'.");
    }
}
